...
MODULE_DESCRIPTION("lkmasg2 Kernel Modle"); ///< The description -- see modinfo
...
{
	printk(KERN_INFO "read stub"); //Checking to see if it is working (test)
	
	//Error handling
	int error = copy_to_user(buffer, message, length);
	if (error == 0)
	{
		printk(KERN_INFO "Message written: [%s].\n", message);
		return(length=0);
	}
	else
	{
	...
	printk(KERN_INFO "Reached the end of read stub"); //Checking to see if it is working (test)
	return 0;
}
...
{
	printk(KERN_INFO "write stub"); //Checking to see if it is working (test)
	
	sprintf(message, "%s", buffer);
	length = strlen(message);
	
	printk(KERN_INFO "Message written: [%s]", message);
	printk(KERN_INFO "End of writing stub"); //Checking to see if it the function ends working (test)
